...
public class BoardDao {
	...
	//댓글 리스트 가져오기
	public ArrayList<BoardComment> listComment(int boardno){
		Connection conn = null;		
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			conn = ConnectionHelper.getConnection("oracle");
			String sql = "select commentno, boardno, writer, content, regdate "
			...
			while(rs.next()) {
				...
				comment.setRegDate(df.format(rs.getDate(5)));
